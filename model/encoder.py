...
class BertPolyDssmModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        responses_segment_ids = responses_segment_ids.view(-1, seq_length)
        if isinstance(self.bert, DistilBertModel):
            state_vecs = self.bert(responses_input_ids, responses_input_masks)[
                -1
            ]  # [bs, length, dim]
        else:
            state_vecs = self.bert(
    ...
    def encoding(
        ...
        if labels is not None:
            ...
            cos_similarity = (dot_product + 1) / 2
            return cos_similarity
    def embed_q(self,context_input_ids,context_segment_ids,context_input_masks=None):
      batch_size, seq_length = context_input_ids.shape
      res_cnt = 1
      state_vecs = self.bert(
                context_input_ids, context_input_masks, context_segment_ids
            )[
      ...
      context_vecs = F.normalize(context_vecs, 2, -1)  # [bs, m, dim]
      return context_vecs
    def embed_d(self,responses_input_ids,responses_segment_ids,responses_input_masks=None):
      batch_size, seq_length = responses_input_ids.shape
      res_cnt = 1
      state_vecs = self.bert(
          responses_input_ids, responses_input_masks, responses_segment_ids
      )[
